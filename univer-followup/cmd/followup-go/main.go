...
func (a *app) handleSave(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		writeJSON(w, http.StatusBadRequest, apiError{OK: false, Error: "payload 必须是对象"})
		return
	}
	...
	expectedSignature, _ := req["expected_signature"].(string)
	force, _ := req["force"].(bool)
	if expectedSignature != "" && !force {
		state, err := a.remoteState()
		if err != nil {
			writeJSON(w, http.StatusInternalServerError, apiError{OK: false, Error: err.Error()})
			return
		...
		if currentSignature != "" && currentSignature != expectedSignature {
	...
	seatable, err := a.syncToSeaTable(payload)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, apiError{OK: false, Error: err.Error()})
		return
	}
	savedAt, err := a.savePayload(payload, req["snapshot"])
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, apiError{OK: false, Error: err.Error()})
		return
	}
