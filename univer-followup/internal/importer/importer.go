...
func run(cfg config) error {
	wb, err := loadWorkbook(cfg)
	if err != nil {
		return err
	}
	if cfg.dryRun {
		tableName := firstNonEmpty(cfg.tableName, wb.SheetName, "随访记录")
		fmt.Printf("dry run: table=%s sheet=%s columns=%d rows=%d\n", tableName, wb.SheetName, len(wb.Headers), len(wb.Rows))
		return nil
	}
	...
	plan, err := c.prepareImport(wb)
	if err != nil {
		return err
	}

	backupDir, err := c.coldBackup(plan.Metadata, plan.Existing, cfg.xlsxPath)
	if err != nil {
		return err
	}
	fmt.Println("cold backup:", backupDir)

	if err := c.addTable(plan.CreateName, plan.Workbook); err != nil {
		return err
	}
	fmt.Printf("created table: %s (%d columns, %d rows)\n", plan.CreateName, len(wb.Headers), len(wb.Rows))

	if err := c.deleteExistingTables(plan); err != nil {
		return err
	}
	if err := c.finalizeImport(plan); err != nil {
		return err
	}
	return c.verifyImportedRows(plan)
}

func loadWorkbook(cfg config) (parsedWorkbook, error) {
	wb, err := parseXLSX(cfg.xlsxPath)
	if err != nil {
		return parsedWorkbook{}, err
	}
	if len(wb.Headers) == 0 {
		return parsedWorkbook{}, errors.New("xlsx header row is empty")
	}
	return wb, nil
}

func (c *client) prepareImport(wb parsedWorkbook) (importPlan, error) {
	targetName := firstNonEmpty(c.cfg.tableName, wb.SheetName, "随访记录")
	meta, err := c.metadata()
	if err != nil {
		return importPlan{}, err
	}
	existing := metadataTableNames(meta)
	if contains(existing, targetName) && !c.cfg.replaceExisting {
		return importPlan{}, fmt.Errorf("目标表已存在，拒绝覆盖: %s", targetName)
	}
	createName := targetName
	if contains(existing, targetName) && c.cfg.replaceExisting {
		createName = targetName + "_导入中_" + time.Now().Format("150405")
	}
	return importPlan{
		Workbook:   wb,
...
func (c *client) deleteExistingTables(plan importPlan) error {
	if !c.cfg.deleteExisting {
		return nil
	}
	for _, name := range plan.Existing {
		if name == plan.CreateName {
			continue
		}
...
func (c *client) finalizeImport(plan importPlan) error {
	if plan.CreateName == plan.TargetName {
		return nil
	}
	if err := c.renameTable(plan.CreateName, plan.TargetName); err != nil {
		return err
	}
	fmt.Printf("renamed table: %s -> %s\n", plan.CreateName, plan.TargetName)
	return nil
}

func (c *client) verifyImportedRows(plan importPlan) error {
	rows, err := c.rows(plan.TargetName)
	if err != nil {
		return err
	}
	if len(rows) != len(plan.Workbook.Rows) {
		return fmt.Errorf("row count mismatch after import: SeaTable=%d xlsx=%d", len(rows), len(plan.Workbook.Rows))
	}
